...
from src.agents.buffers.numpy_buffer import NumpyBuffer

OBSERVATION_SHAPE = (4, 1, 84, 84)
OBS_DTYPE = np.uint8
STACK_SIZE = 4
BATCH_SIZE = 5
N_STEP = 5
...
class ReplayBufferRegressionTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        cfg = compose(config_name='ddqn_numpy')
        ...
        buffer = NumpyBuffer(device=DEVICE, gamma=cfg['gamma'], **self.buffer_cfg)
        self.memory = buffer
    ...
    def testSampleTransitionBatch(self):
        ...
        memory = NumpyBuffer(device=DEVICE, gamma=0.99, **self.buffer_cfg)
        nums_add = 50
        index_to_id = []
